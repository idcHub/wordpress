...
public function plugin_action_links( $links, $file ) {
	...
	if ( $file == $this_plugin ) {
		...
		$del_setting_deactivation_link = '<a href="' . $link . '">설정을 삭제해서 정지</a>';
...
<div class="form-field">
	<label for="meta_description">메타 디스크립션</label>
...
?>
	<tr class="form-field">
		<th scope="row" valign="top"><label for="meta_keywords">메타 키워드</label></th>
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label for="meta_description">메타 디스크립션</label></th>
...
public function add_post_meta_box( $post_type, $post ) {
	...
	if ( $post_type_obj && $post_type_obj->public && $output_flag ) {
		add_meta_box( 'post_meta_box', '메타 정보', array( &$this, 'post_meta_box' ), $post_type, 'normal', 'high');
	}
}
			...
			<tr>
				...
				<td>
					<label for="excerpt_as_description">
						...
						요약을 포스트 디스크립션으로 이용하기
					</label>
				</td>
			</tr>
		</table>
		<h3>Taxonomy(분류) 설정</h3>
		<table class="form-table">
			<tr>
				<th>Taxonomy 메타 키워드</th>
				<td>
